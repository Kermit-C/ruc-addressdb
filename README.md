...
本实验的数据库要求实现通信簿，故本系统的关系模式设置为如下，并且以二进制文件的形式存储在了项目根目录下的 `meta` 元数据文件中：
...
由于实验3的项目较为完善，实验4是在实验3的基础上增加了元数据的功能，实验4在本仓库中标注了tag为：`expe.4` （实验3的版本为：`expe.3` ）

可以通过 Git History 查看实验4新增部分的代码。
...
```other
# 使用当前文件夹下的 data 数据作为通信薄数据源，没有则会自动创建
# 默认使用当前文件夹下的 meta 数据作为元数据输入
addressdb

# 使用 p 参数指定通信薄数据源的路径，支持同一个计算机上存在多个数据源
# 使用 m 参数指定元数据的路径
addressdb -p <data path> -m <metadata path>
...
![Image.png](https://res.craft.do/user/full/023b9e5b-4b7b-73bd-d0b7-3bfd020522ed/doc/A09A6299-BD35-418C-A325-664C3A660740/6D13CDE9-ACBD-47F4-9235-48060F3C6D19_2/rfruYdmZWaqpaYf0EA3ub4pLz3yvSdJjNsXK8hX016Uz/Image.png)
...
查询所有数据的示例如下，使用的数据源为上方 insert 操作插入的数据

![Image.png](https://res.craft.do/user/full/023b9e5b-4b7b-73bd-d0b7-3bfd020522ed/doc/A09A6299-BD35-418C-A325-664C3A660740/820E4FDC-681E-4D73-A476-1B68F4E0577B_2/9SUnhfaWomQK4HRhc5vlYXzFZYYJtvpizvuSJc8Leqcz/Image.png)

也可以显示所有字段

![Image.png](https://res.craft.do/user/full/023b9e5b-4b7b-73bd-d0b7-3bfd020522ed/doc/A09A6299-BD35-418C-A325-664C3A660740/95EF1AF1-97BA-4C13-B8B9-43E95BFB4685_2/4WWik9aDT4HS21U9rZybAgVUa9wPRrBLrgaz1mu7z58z/Image.png)
...
└── src                 # 代码
    ...
    ├── schema.c        # 关系模式相关的模块，用于从元数据获取关系模式、提供关系模式查询、字段查询、往缓冲区读写等
...
元数据将会保存为二进制文件，默认为当前目录的 meta 文件，也可以通过 -m 参数自行指定。
